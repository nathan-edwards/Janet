    ...
    async run(client, ctx, args) {
        ...
        if (subCommand === "add") {
            ...
            if (isExRole)
                return await ctx.sendMessage({
                    embeds: [
                        embed.setDescription(`The dj role <@&${roleId}> is already added`),
                    ],
                });
            if (!dj) {
                ...
                return await ctx.sendMessage({
                    embeds: [
                        embed.setDescription(`The dj role <@&${roleId}> has been added`),
                    ],
                });
            }
            else {
            ...
            if (!isExRole)
                return await ctx.sendMessage({
                    embeds: [
                        embed.setDescription(`The dj role <@&${roleId}> is not added`),
                    ],
                });
            await this.client.prisma.roles.delete({ where: { roleId: roleId } });
            return await ctx.sendMessage({
                embeds: [
                    embed.setDescription(`The dj role <@&${roleId}> has been removed`),
                ],
            });
        }
