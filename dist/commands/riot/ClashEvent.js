...
export default class ClashEvent extends Command {
    ...
    async run(client, ctx, args) {
        ...
        const data = await RiotAPI(args[0], "/lol/clash/v1/tournaments");
        for (let i = 0; i < data.length; i++) {
            for (let j = 0; j < data.length - i - 1; j++) {
                if (data[j + 1].schedule[0].startTime < data[j].schedule[0].startTime) {
...
//# sourceMappingURL=ClashEvent.js.map