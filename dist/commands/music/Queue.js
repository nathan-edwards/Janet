...
export default class Queue extends Command {
    ...
    async run(client, ctx, args) {
        ...
        const data = await searchSong(player.current.info.title);
        ...
        const queue = player.queue.map((track, index) => `${index + 1}. [${track.info.title}](${track.info.uri}) - Request By: ${track?.info.requester} - Duration: ${track.info.isStream
            ? "LIVE"
